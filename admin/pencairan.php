...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($rental_id > 0 && $aksi === 'cairkan_dana') {
        ...
        $adaPending = (int) $cekLaporan->fetchColumn();

        if ($adaPending > 0) {
            // Ada laporan pending → blokir pencairan
            $tab = $_GET['tab'] ?? 'belum';
            header("Location: pencairan.php?tab=$tab&blocked=1");
            exit;
        }

        // ── Hitung nominal dengan ganti rugi ──
        $rent = $conn->prepare("
...
$sql = "
    SELECT r.*,
           ...
           rp.tagihan_ganti_rugi    AS gr_alasan
    FROM rentals r
    JOIN items i  ON r.item_id = i.id
    JOIN users u  ON r.user_id = u.id
    JOIN users pu ON i.user_id = pu.id
    LEFT JOIN reports rp ON rp.target_id = r.id AND rp.status != 'dismissed'
...
.topbar h1{font-size:17px;font-weight:600}.topbar p{font-size:12px;color:#6b7280;margin-top:1px}
...
.stat-icon.purple{background:#f5f3ff;color:#7c3aed}.stat-icon.green{background:#e9f9f0;color:#16a34a}
.stat-icon.amber{background:#fff7e6;color:#cc7a00}.stat-icon.blue{background:#eef0ff;color:#3d4bff}
.stat-label{font-size:11.5px;color:#6b7280;margin-bottom:4px}.stat-value{font-size:20px;font-weight:800;line-height:1}.stat-value.sm{font-size:14px}
...
.tab-pill{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:20px;white-space:nowrap;font-size:13px;font-weight:600;border:1.5px solid #e5e7eb;color:#6b7280;background:#fff;transition:all .15s}
.tab-pill:hover{border-color:#7c3aed;color:#7c3aed}.tab-pill.active{background:#7c3aed;border-color:#7c3aed;color:#fff}
.tc{font-size:10.5px;font-weight:700;background:#f0f1f3;color:#6b7280;border-radius:20px;padding:1px 7px}
.tab-pill.active .tc{background:rgba(255,255,255,.25);color:#fff}
...
.btn-export{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;background:#7c3aed;color:#fff;border:none;border-radius:9px;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;transition:background .15s;white-space:nowrap;margin-left:auto}
.btn-export:hover{background:#6d28d9}.btn-export i{font-size:17px}
...
.pc-card.urgent{border-left:4px solid #7c3aed}.pc-card.done{border-left:4px solid #16a34a}
.pc-card.blocked{border-left:4px solid #f59e0b}
...
.pc-thumb img{width:100%;height:100%;object-fit:cover}.pc-thumb i{font-size:22px;color:#d1d5db}
.pc-info{flex:1;min-width:0}.pc-name{font-size:14px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.pc-meta{font-size:12px;color:#6b7280;margin-top:3px;display:flex;gap:10px;flex-wrap:wrap;align-items:center}.pc-meta i{font-size:13px}
.pc-right{display:flex;flex-direction:column;align-items:flex-end;gap:6px;flex-shrink:0}
.pc-total{font-size:16px;font-weight:800;color:#7c3aed}.pc-id{font-size:11px;color:#9ca3af}
...
.sb-blocked{background:#fff7e6;color:#a16207;border:1px solid #fed7aa}
...
.pc-cell-label{font-size:10.5px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em}
...
.nominal-label{font-size:10.5px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.04em}
.nominal-val{font-size:14px;font-weight:800}
.nominal-val.total{color:#1a1d2e}.nominal-val.komisi{color:#dc2626}.nominal-val.dicairkan{color:#7c3aed}
.nominal-val.deduction{color:#d97706}
...
.tujuan-row i{font-size:18px;color:#7c3aed;flex-shrink:0}.tujuan-detail{flex:1}
.tujuan-title{font-size:12px;font-weight:700;color:#6d28d9;margin-bottom:2px}
.tujuan-val{font-size:13.5px;font-weight:800;color:#1a1d2e}.tujuan-sub{font-size:11.5px;color:#6b7280}
...
.qris-title{font-size:12px;font-weight:700;color:#6d28d9;display:flex;align-items:center;gap:6px}.qris-title i{font-size:16px}
.qris-name{font-size:14px;font-weight:800;color:#1a1d2e;margin-top:2px}.qris-sub{font-size:11.5px;color:#6b7280}
.qris-img-wrap{flex-shrink:0}
.qris-img{width:130px;height:130px;object-fit:contain;border-radius:10px;border:2px solid #ddd6fe;background:#fff;cursor:pointer;transition:transform .15s,box-shadow .15s}
.qris-img:hover{transform:scale(1.03);box-shadow:0 4px 16px rgba(124,58,237,.18)}
.qris-img-label{font-size:10px;font-weight:700;color:#7c3aed;text-align:center;margin-top:5px;letter-spacing:.04em}
...
.lightbox.active{display:flex}.lightbox img{max-width:90vw;max-height:88vh;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.5)}
.lightbox-close{position:absolute;top:18px;right:22px;background:rgba(255,255,255,.15);color:#fff;border:none;border-radius:50%;width:36px;height:36px;font-size:20px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s}
...
.done-row i{font-size:18px;color:#16a34a;flex-shrink:0}.done-detail{flex:1}
...
/* Laporan pending warning */
.laporan-block-row{padding:12px 18px;border-bottom:1px solid #f0f1f3;background:#fffbeb;display:flex;align-items:flex-start;gap:10px;flex-wrap:wrap}
.laporan-block-row i{font-size:18px;color:#d97706;flex-shrink:0;margin-top:1px}
.laporan-block-detail{flex:1}
.laporan-block-title{font-size:12.5px;font-weight:700;color:#92400e;margin-bottom:3px}
.laporan-block-sub{font-size:12px;color:#a16207;line-height:1.4}
.laporan-block-sub strong{color:#78350f}

/* Tambahan ganti rugi info */
.gr-deduction-row{padding:10px 18px;border-bottom:1px solid #f0f1f3;background:#e9f9f0;display:flex;align-items:center;gap:8px}
.gr-deduction-row i{font-size:16px;color:#16a34a;flex-shrink:0}
.gr-deduction-row span{font-size:12.5px;color:#166534;font-weight:600}
...
.btn-cairkan{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:9px;background:#7c3aed;color:#fff;border:none;font-family:inherit;font-size:13px;font-weight:700;cursor:pointer;transition:background .15s}
.btn-cairkan:hover{background:#6d28d9}.btn-cairkan i{font-size:15px}
.btn-cairkan:disabled{background:#d1d5db;color:#9ca3af;cursor:not-allowed}
...
/* Blocked alert */
.blocked-alert{background:#fffbeb;border:1px solid #fde68a;border-radius:12px;padding:14px 18px;margin-bottom:18px;display:flex;align-items:flex-start;gap:10px}
.blocked-alert i{font-size:20px;color:#d97706;flex-shrink:0;margin-top:2px}
.blocked-alert div{font-size:13px;color:#92400e;line-height:1.5}
.blocked-alert strong{color:#78350f}

@media(max-width:860px){.stats-grid{grid-template-columns:repeat(2,1fr)}.pc-body{grid-template-columns:1fr 1fr}}
@media(max-width:580px){.pc-body{grid-template-columns:1fr}.pc-cell{border-right:none;border-bottom:1px solid #f0f1f3}.qris-img{width:100px;height:100px}.content{padding:16px}}
</style>
</head>
<body>
...
<div class="main">
    <div class="topbar">
        <div><h1>Pencairan Dana</h1><p>Transfer hasil sewa ke pemilik barang setelah transaksi selesai</p></div>
        <div class="topbar-right"><span class="admin-pill">Admin</span><div class="avatar"><?= strtoupper(substr($_SESSION['username'] ?? 'A', 0, 1)) ?></div></div>
    </div>
    <div class="content">
        ...
        <div class="blocked-alert">
            ...
            <div><strong>Pencairan diblokir!</strong> Masih ada laporan pending pada rental ini. Selesaikan laporan terlebih dahulu di halaman <a href="reports.php" style="color:#78350f;text-decoration:underline;font-weight:700;">Kelola Laporan</a> sebelum pencairan bisa dilakukan.</div>
        </div>
        <?php endif; ?>

        <div class="stats-grid">
            <div class="stat-card"><div class="stat-icon purple"><i class="ti ti-wallet"></i></div><div><div class="stat-label">Perlu Dicairkan</div><div class="stat-value"><?= $total_belum ?></div></div></div>
            <div class="stat-card"><div class="stat-icon amber"><i class="ti ti-cash"></i></div><div><div class="stat-label">Total Nilai Tertahan</div><div class="stat-value sm">Rp <?= number_format($total_nilai_belum, 0, ',', '.') ?></div></div></div>
            <div class="stat-card"><div class="stat-icon green"><i class="ti ti-circle-check"></i></div><div><div class="stat-label">Sudah Dicairkan</div><div class="stat-value"><?= $total_sudah ?></div></div></div>
            <div class="stat-card"><div class="stat-icon blue"><i class="ti ti-trending-up"></i></div><div><div class="stat-label">Total Dicairkan</div><div class="stat-value sm">Rp <?= number_format($total_nilai_sudah, 0, ',', '.') ?></div></div></div>
        </div>

        <form method="GET" action="" id="filterForm">
            <input type="hidden" name="tab" value="<?= htmlspecialchars($tab) ?>">
            <div class="toolbar">
                <div class="search-box"><i class="ti ti-search"></i><input type="text" name="q" placeholder="Cari barang, penyewa, pemilik..." value="<?= htmlspecialchars($search) ?>"></div>
        ...
        <div class="export-bar">
            <div class="export-bar-label"><i class="ti ti-file-spreadsheet"></i> Export Laporan Pencairan</div>
            <div class="export-sep"></div>
            <form method="GET" action="export_pencairan.php" target="_blank" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;flex:1;">
                <div class="export-input-wrap">
                    <div class="export-input-group"><label>Dari</label><input type="date" name="dari" class="export-input" value="<?= date('Y-m-01') ?>" max="<?= date('Y-m-d') ?>"></div>
                    <div class="export-input-group"><label>Sampai</label><input type="date" name="sampai" class="export-input" value="<?= date('Y-m-d') ?>" max="<?= date('Y-m-d') ?>"></div>
                    <div class="export-input-group"><label>Status</label>
                ...
                <button type="submit" class="btn-export"><i class="ti ti-download"></i> Download Excel</button>
            </form>
        </div>
        ...
        <div class="tab-bar">
            ...
            ?>
            <a href="?tab=<?= $key ?><?= $search ? '&q='.urlencode($search) : '' ?>" class="tab-pill <?= $active ?>">
        ...
        <?php if (empty($rows)): ?>
        <div class="empty-state"><i class="ti ti-wallet-off"></i><h3>Tidak ada data pencairan di kategori ini</h3></div>
        <?php else: ?>
        <div class="pcairan-list">
            <?php foreach ($rows as $r):
                ...
                $grDeduction = (int) ($r['ganti_rugi_deduction'] ?? 0);
                $dicairkan = $tot - $komisi + $grDeduction;
                ...
                $is_qris    = $has_metode && strtolower($r['pemilik_penyedia'] ?? '') === 'qris' && !empty($r['pemilik_foto_qris']);

                $qris_path = null;
                if ($is_qris) {
                    $try = "../uploads/" . $r['pemilik_foto_qris'];
                    if (file_exists($try)) $qris_path = $try;
                }

                $prCount = $rental_pending_map[$r['id']] ?? 0;
                $isBlocked = ($spc === 'ada_laporan') || $prCount > 0;

                $badge = $spc === 'sudah_dicairkan'
                    ? '<span class="sbadge sb-dicairkan"><i class="ti ti-circle-check"></i> Sudah Dicairkan</span>'
                    : ($isBlocked
                        ? '<span class="sbadge sb-blocked"><i class="ti ti-alert-triangle"></i> Diblokir Laporan</span>'
                        : ($has_metode
                ...
                $card_cls = $spc === 'sudah_dicairkan' ? 'done' : ($isBlocked ? 'blocked' : 'urgent');
            ?>
            <div class="pc-card <?= $card_cls ?>">
                <div class="pc-head">
                    <div class="pc-thumb"><?php if ($g): ?><img src="<?= htmlspecialchars($g) ?>" alt=""><?php else: ?><i class="ti ti-box-seam"></i><?php endif; ?></div>
                    <div class="pc-info">
                ...
                <div class="pc-body">
                    <div class="pc-cell"><div class="pc-cell-label">Penyewa</div><div class="pc-cell-val"><div class="av-mini" style="background:<?= $cp[0] ?>;color:<?= $cp[1] ?>;"><?= $ip ?></div><?= htmlspecialchars($r['penyewa']) ?></div></div>
                    <div class="pc-cell"><div class="pc-cell-label">Pemilik Barang</div><div class="pc-cell-val"><div class="av-mini" style="background:<?= $co[0] ?>;color:<?= $co[1] ?>;"><?= $io ?></div><?= htmlspecialchars($r['pemilik']) ?></div></div>
                    <div class="pc-cell"><div class="pc-cell-label">Periode Sewa</div><div class="pc-cell-val" style="font-size:12.5px;"><i class="ti ti-calendar-event"></i> <?= date('d M Y', strtotime($r['tanggal_mulai'])) ?> → <?= date('d M Y', strtotime($r['tanggal_selesai'])) ?></div></div>
                </div>

                <!-- Nominal breakdown -->
                <div class="nominal-row">
                    <div class="nominal-item"><div class="nominal-label">Total Sewa</div><div class="nominal-val total">Rp <?= number_format($tot, 0, ',', '.') ?></div></div>
                    <span class="nominal-sep">−</span>
                    <div class="nominal-item"><div class="nominal-label">Komisi Admin (5%)</div><div class="nominal-val komisi">Rp <?= number_format($komisi, 0, ',', '.') ?></div></div>
                    <?php if ($grDeduction > 0): ?>
                    <span class="nominal-sep">+</span>
                    <div class="nominal-item"><div class="nominal-label">Ganti Rugi (dari Penyewa)</div><div class="nominal-val" style="color:#16a34a;">Rp <?= number_format($grDeduction, 0, ',', '.') ?></div></div>
                    <?php endif; ?>
                    <span class="nominal-sep">=</span>
                    <div class="nominal-item"><div class="nominal-label">Ditransfer ke Pemilik</div><div class="nominal-val dicairkan">Rp <?= number_format($dicairkan, 0, ',', '.') ?></div></div>
                </div>

                <?php if ($grDeduction > 0): ?>
                <div class="gr-deduction-row">
                    <i class="ti ti-alert-circle"></i>
                    <span>Tambahan ganti rugi Rp <?= number_format($grDeduction, 0, ',', '.') ?> — <?= htmlspecialchars(!empty($r['gr_alasan']) ? mb_substr($r['gr_alasan'], 0, 80) : 'Lihat detail di halaman laporan') ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="done-row">
                    ...
                    <div class="done-detail">
                        ...
                        <div class="done-sub">
                            <?php if (!empty($r['tanggal_pencairan'])): ?><span><i class="ti ti-calendar"></i> <?= date('d M Y H:i', strtotime($r['tanggal_pencairan'])) ?></span><?php endif; ?>
                            <?php if ($has_metode): ?><span><i class="ti ti-credit-card"></i> <?= htmlspecialchars($r['pemilik_penyedia']) ?><?php if (!$is_qris): ?> — <?= htmlspecialchars($r['pemilik_rekening']) ?><?php endif; ?></span><?php endif; ?>
                        </div>
                    </div>
                    <?php if (!empty($r['bukti_pencairan'])): ?><a href="../uploads/bukti/<?= htmlspecialchars($r['bukti_pencairan']) ?>" target="_blank" class="btn-bukti"><i class="ti ti-eye" style="font-size:13px;"></i> Lihat Bukti</a><?php endif; ?>
                </div>

                <?php elseif ($isBlocked): ?>
                <!-- Diblokir oleh laporan pending -->
                <div class="laporan-block-row">
                    <i class="ti ti-alert-triangle"></i>
                    <div class="laporan-block-detail">
                        <div class="laporan-block-title"><i class="ti ti-flag"></i> Pencairan Diblokir — <?= $prCount ?> Laporan Pending</div>
                        <div class="laporan-block-sub">Admin harus <strong>menyelesaikan atau menolak</strong> semua laporan pada rental ini sebelum pencairan bisa dilakukan. <a href="reports.php" style="color:#78350f;text-decoration:underline;">Buka halaman laporan →</a></div>
                    </div>
                </div>
                ...
                <div class="qris-row">
                    <div class="qris-left">
                        ...
                        <div class="qris-sub">Scan QR di samping untuk transfer — Rp <?= number_format($dicairkan, 0, ',', '.') ?></div>
                    </div>
                    <div class="qris-img-wrap">
                        <?php if ($qris_path): ?>
                        <img src="<?= htmlspecialchars($qris_path) ?>" alt="QR Code" class="qris-img" onclick="openLightbox('<?= htmlspecialchars($qris_path) ?>')" title="Klik untuk perbesar">
                        <div class="qris-img-label">KLIK UNTUK PERBESAR</div>
                        <?php else: ?>
                        <div style="width:130px;height:130px;border-radius:10px;border:2px dashed #ddd6fe;background:#f5f3ff;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;color:#9ca3af;font-size:12px;text-align:center;padding:8px;"><i class="ti ti-photo-off" style="font-size:24px;color:#c4b5fd;"></i>File QR tidak ditemukan</div>
                        <?php endif; ?>
                    </div>
                </div>
                <form method="POST" enctype="multipart/form-data" class="pc-footer">
                    ...
                    <button type="submit" class="btn-cairkan" onclick="return confirm('Konfirmasi: Rp <?= number_format($dicairkan, 0, ',', '.') ?> sudah ditransfer via QRIS ke <?= addslashes($r['pemilik_nama_rek'] ?? $r['pemilik']) ?>?')"><i class="ti ti-send"></i> Tandai Sudah Dicairkan</button>
                </form>
                ...
                <form method="POST" enctype="multipart/form-data" class="pc-footer">
                    ...
                    <label>Bukti transfer:</label>
                    <input type="file" name="bukti_pencairan" accept=".jpg,.jpeg,.png,.pdf" required>
                    <button type="submit" class="btn-cairkan" onclick="return confirm('Konfirmasi: Rp <?= number_format($dicairkan, 0, ',', '.') ?> sudah ditransfer ke <?= addslashes($r['pemilik_nama_rek'] ?? $r['pemilik']) ?>?')"><i class="ti ti-send"></i> Tandai Sudah Dicairkan</button>
                </form>

                <?php else: ?>
                <div class="nometode-row"><i class="ti ti-alert-circle"></i> Pemilik belum setup metode pembayaran — belum bisa dicairkan</div>
                <?php endif; ?>
            </div>
            <?php endforeach; ?>
        </div>
...
<script>
function openLightbox(s){document.getElementById('lightbox-img').src=s;document.getElementById('lightbox').classList.add('active');document.body.style.overflow='hidden'}
